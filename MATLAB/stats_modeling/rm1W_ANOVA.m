function [outTableANOVA, within_StdErr, outTableMC] = rm1W_ANOVA(data,VariableNames,saveOut,graph,titlegraph)
...
if graph==1
    ...
    bar(mean(data),'FaceColor',[1, 1, 1])
    hold on

%     for ibin = 1:size(data, 2)
%         
%         xvect = ibin + randn(length(data), 1)/10;
%         yvect = data(:, ibin);
%         scatter(xvect, yvect, 10, [.5, .5, .5], 'filled')
% 
%     end

    errorbar(cndl_means,within_StdErr,'.k', 'LineWidth', 3)
    set(gca,'fontsize',14)
    set(gca, 'XTickLabel',VariableNames, 'XTick',1:numel(VariableNames),'FontSize',13)
    title(titlegraph, 'FontSize',15)
